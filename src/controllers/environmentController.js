...
exports.getEnvironment = async (req, res, next) => {
  try {
    ...
    req.log.error("Failed to get environment", {
...
exports.updateEnvironment = async (req, res, next) => {
  try {
    ...
    req.log.error("Failed to update environment", {
      environmentId: req.params.id,
      error: error.message,
    });
    next(error);
...
exports.deleteEnvironment = async (req, res, next) => {
  try {
    ...
    req.log.error("Failed to delete environment", {
      environmentId: req.params.id,
      error: error.message,
    });
    next(error);
...
exports.pushInfrastructure = async (req, res, next) => {
  try {
    const { id } = req.params;
    let user = await userService.getUserByKeycloakId(req.user.id);
    if (!user) {
...
exports.createTerraformBackend = async (req, res, next) => {
  try {
    ...
    if (!cloudCredentialId) {
      return res
        .status(400)
        .json({ error: "AWS credentials are required to create Terraform backend resources" });
    }
    ...
    try {
      ...
      req.log.error("Failed to decrypt credentials", {
        credentialId: cloudCredentialId,
        error: error.message,
      });
      return res
        .status(400)
        .json({ error: "Failed to decrypt cloud credentials. Please update your credentials." });
    }

    const awsAccessKeyId = decryptedCredentials?.accessKeyId || decryptedCredentials?.accessKey;
    const awsSecretAccessKey =
      decryptedCredentials?.secretAccessKey || decryptedCredentials?.secretKey;
    ...
    if (!awsAccessKeyId || !awsSecretAccessKey) {
      ...
      return res.status(400).json({
        error:
          "Cloud credential does not contain valid AWS access keys. Please update your credentials.",
      });
    }
